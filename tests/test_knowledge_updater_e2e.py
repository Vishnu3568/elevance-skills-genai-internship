...
class TestKnowledgeUpdaterE2E(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Source paths
        real_kb_path = os.path.join(self.base_dir, "dataset", "knowledge_base.csv")
        real_faiss_dir = os.path.join(self.base_dir, "faiss_index")
        self.updates_path = os.path.join(self.base_dir, "dataset", "knowledge_updates.csv")

        # Copy original KB and FAISS to temporary isolated workspace
        shutil.copyfile(real_kb_path, self.temp_kb_path)
        shutil.copytree(real_faiss_dir, self.temp_faiss_dir)
